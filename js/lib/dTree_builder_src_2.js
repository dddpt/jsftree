...
class TreeBuilder {
  ...
  constructor(root, siblings, opts) {
    ...
    // remove "duplicates"
    // -> wrong spot, the removed node will still be referenced as children of their parent
    // -> need to be done at the preprocess stage in dTree
    /*tballnodes = this.allNodes
    tballnodes_copy = JSON.parse(JSON.stringify(JSON.decycle(this.allNodes)))
    this.uniqueNodes = new Set()
    this.duplicateNodes = _.partition(this.allNodes,function(node){
      tr = uniqueNodes.has(node.data.name)
      uniqueNodes.add(node.data.name)
      return(tr)
    })
    this.allNodes = this.duplicateNodes[0]
    this.duplicateNodes = this.duplicateNodes[1]*/
  ...
  _update(source) {
    ...
    tblinks = links
    tblinks_copy = JSON.parse(JSON.stringify(JSON.decycle(links)))
