...
var transitionsDuration = 800 // duration of transitions in msec
...
function createFamilyTree(svgContainerSelector,ftreeData){
  ...
  return updateFamilyTree(ftreeData,undefined);
}
...
function updateFamilyTree(ftreeData, source){
  ...
  var ftree, nodes, links;

  ftree = d3FamilyTree(ftreeData,{width:100,height:100},{width:50,height:50})
  ftree.applyForces()
  ...
  if(!source){
    source = {x:ftree.width()/2,y:ftree.height()/2}
  }
...
var resp = $.get( "data/example_gedcom.ged" ,function(data){
  ...
  createFamilyTree("#graph",d3ized_ged,true)
}  );
// ==================== FILE DROP ====================
